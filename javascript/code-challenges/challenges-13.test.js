...
// Solution code here...
  var test;
  arr.reduce((bank, current) => bank.length > current.length ? test = bank : test = current,'');
...
const findHappiness = (arr) => {
  // Solution code here...
  const happy =[];
  arr.forEach( str => str.includes(":)") ? happy.push(str) : happy);
  return happy;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let odd =[];
  str.split('').reduce( (bank, current, i) => i % 2 === 1 ? odd.push(current) : odd, []);
  return odd.join('');
};
